<div class="container mx-auto px-4">
    <h1 class="text-2xl font-bold mb-4 text-white">List Unit Kerja</h1>
        ...
        <table class="min-w-full border-collapse mt-4 text-center">
            <thead>
                <tr class="bg-gray-800 text-white">
                    ...
                    <th class="py-3 px-4 border border-gray-600">Kode</th>
            ...
            <tbody>
                @foreach ($unitKerjas as $index => $unit)
                    <tr class="{{ $index % 2 === 0 ? 'bg-gray-900' : 'bg-gray-800' }} text-white">
                        <td class="py-2 px-4 border border-gray-700">{{ $unit->id }}</td>
                        <td class="py-2 px-4 border border-gray-700">{{ $unit->kode }}</td>
                        <td class="py-2 px-4 border border-gray-700">{{ $unit->nama }}</td>
